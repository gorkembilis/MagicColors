...
export default function Gallery() {
  ...
  const savedArt = [
    ...
    { id: 3, src: packs[4].cover, date: "yesterday", prompt: "Princess castle" },
  ];
  const { t } = useI18n();
      ...
      <div className="p-4">
        {savedArt.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <Button className="rounded-full px-8 font-bold">{t('gallery.createNow')}</Button>
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-4">
            {savedArt.map((art) => (
              <Card key={art.id} className="overflow-hidden border-none shadow-sm">
                ...
                <div className="p-3">
                  <p className="text-xs font-medium text-muted-foreground mb-2">{t(`gallery.${art.date}`)}</p>
